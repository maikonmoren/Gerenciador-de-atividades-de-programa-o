...
<html>
  ...
  <div class="input-group-prepend">
    <span class="btn btn-dark active" id="inputGroup-sizing-default" disable>Selecione a sala</span>
  </div>
                      <select type="" id="busca" class="form-control col-sm-10">
                          <option value="T">Selecione...</option>
 <?php          $pdo = Connection::getConexao();
                        $sql = 'select t_nome,t_codigo from tb_turma where t_codigo in(select t_codigo from pe_tu where p_id = '.$_SESSION['p_id'].') ;';
                        foreach($pdo->query($sql)as $linha)
                        {
                            echo '<option value='.$linha['t_codigo'].'>'.$linha['t_nome'].'</option>';
      
        }?>
                      </select>
</div><hr>
         <?php          $pdo = Connection::getConexao();
                        $sql = 'select a.a_id , a.a_info, a.a_titulo , a.a_data ,a.t_codigo, pa.pe_at_situacao as st, pa.pe_at_feedback from tb_atividade a inner join pe_at pa 
                                where a.a_id = pa.a_id and pa.p_id ='.$_SESSION['p_id'].';';
                        foreach($pdo->query($sql)as $row)
                        {
                            ...
                            echo '<div class="card-text"><small>'.$row['a_info'].'</small></div>';
                            echo '<p class="card-text"><small>Entregar até &nbsp;'.$row['a_data'].'</small></p>';
                            if($row['st']>0){
                                echo '<div>Retorno :</div>';
                                echo '<hr>';
                               echo '<div class="card-text">'.$row['pe_at_feedback'].'</div><br>';
                               
                               echo ' <button" disabled class="btn btn-secondary disabled"></i>Atividade enviada &nbsp;<i class="fas fa-check"></i> </button>';
                               echo ' <a href="../action/visualiza_atividade.php?id='.$row['a_id'].'"><button" disabled class="btn btn-success"></i>Ver código enviado &nbsp;<i class="fas fa-copy"></i></button></a>'; 
                            }else{
                                  echo ' <a href="atividade.php?at='.$row['a_id'].'&cd='.$row['t_codigo'].'" class="btn btn-dark"></i>Fazer atividade &nbsp;<i class="fas fa-copy"></i></a> ';
                            }
